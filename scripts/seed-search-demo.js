...
const { getFirestore, FieldValue } = dep('firebase-admin/firestore');
const db = getFirestore();
(async () => {
  const batch = db.batch();
  for (let number = 1; number <= 40; number++) {
    batch.set(db.collection('books').doc(`demo-series-${String(number).padStart(2, '0')}`), {
      ownerId: 'bella', ownerName: 'BellaBooks',
      ...
      seriesNumber: number, status: 'Available', genre: 'Fiction', isbn: '', publishedYear: null,
      condition: 'Good', coverUrl: '', description: '', rating: 4, quietImport: true,
